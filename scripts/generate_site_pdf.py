...
def load_citation_metadata():
    ...
    if not citation_file.exists():
        ...
        return {'version': '0.5', 'codename': 'Caracas', 'title': 'TEP-COS'}
    
    try:
        if yaml:
            with open(citation_file, 'r') as f:
                data = yaml.safe_load(f)
            version_str = str(data.get('version', '0.5'))
        else:
            # Parse manually if yaml not available
            with open(citation_file, 'r') as f:
                content = f.read()
            version_match = re.search(r'^version:\s*"?([^"\n]+)"?', content, re.MULTILINE)
            version_str = version_match.group(1).strip() if version_match else '0.5'
        ...
        if match:
            ...
            codename = match.group(2) or 'Caracas'
        else:
...
def copy_pdf_to_docs(source_pdf: Path, docs_dir: Path):
    ...
    # Primary PDF name
    target_name = f"10-TEP-COS-{version_str}.pdf"
...
async def generate_pdf(quality: str = 'high', wait_time: float = 5.0, skip_build: bool = False):
    ...
    async with HTMLToPDFConverter() as converter:
        ...
        # Process with metadata and compression (do this before root copy so both are compressed)
        process_pdf_with_metadata(final_pdf)
        
        # Copy to root directory (now compressed)
