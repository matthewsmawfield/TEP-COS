...
def analyze_age_discrepancy(df):
    ...
    df_clean = df.dropna(subset=['delta_log_age', 'log_sigma', 'LW_Z_1RE', 'PHOTOMETRIC_MASS'])
    
    X = df_clean[['LW_Z_1RE', 'PHOTOMETRIC_MASS']].values
    y = df_clean['delta_log_age'].values
    
    reg = LinearRegression().fit(X, y)
    df_clean['delta_age_resid'] = y - reg.predict(X)
...
def create_figure(df, results):
    ...
    ax.set_title(f'Age Discrepancy vs $\sigma$ (r={results["r_simple"]:.3f})')
    ...
    plt.tight_layout()
    out_file = os.path.join(FIGURES_DIR, 'sdss_test_e_age_discrepancy.png')
...
def main():
    ...
    out_path = os.path.join(RESULTS_DIR, 'sdss_test_e_results.json')
