#!/usr/bin/env python3
"""
Step 33: Theoretical Uncertainty Framework
============================================

Formal uncertainty quantification for TEP predictions.
...
def compute_theoretical_uncertainties():
    ...
    s33_path = Path('results/outputs/step_12_hierarchical_density_results.json')
    if not s33_path.exists():
        print(f"ERROR: Required input file not found: {s33_path}")
        print(f"Theoretical uncertainty requires hierarchical density results from step_12.")
        raise RuntimeError("Missing required input: step_12_hierarchical_density_results.json")
    ...
    if s48_path.exists():
        with open(s48_path) as f:
            s48 = json.load(f)
        newt_pred = s48.get('comparison', {}).get('cmc_predicted_slope', 0.748)
        newt_err = s48.get('comparison', {}).get('cmc_predicted_error', 0.039)
    else:
        newt_pred = s33_data.get('newtonian_predicted_slope', 0.748)
        newt_err = s33_data.get('newtonian_predicted_error', 0.039)
        print(f"  Warning: {s48_path} not found; using Newtonian slope from step_12.")

    # Estimate propagated uncertainty from model-method variation
    # Use difference between WLS (cluster means) and mixed-effects (individual) slopes
    # as a proxy for model-specification systematic uncertainty
    wls_slope = s33_data.get('model_a_wls_slope', gamma_central)
    mixed_slope = s33_data.get('model_b_mixed_slope', gamma_central)
    gamma_rho_unc = abs(wls_slope - mixed_slope) / 2
    if gamma_rho_unc == 0:
        gamma_rho_unc = 0.03  # Minimum reasonable model-difference uncertainty
    gamma_rho_range = [gamma_central - gamma_rho_unc, gamma_central + gamma_rho_unc]

    # Systematic uncertainty: model specification (period cut, B-proxy treatment)
    # Estimated from variation across period-cut sensitivity analysis (step_02)
    s10_path = Path('results/outputs/step_02_pulsar_population_controls.json')
    if s10_path.exists():
        with open(s10_path) as f:
            s10 = json.load(f)
        sens = s10.get('period_cut_sensitivity', [])
        if sens:
            diffs = [r['period_matched_diff'] for r in sens]
            gamma_sys = np.std(diffs) if len(diffs) > 1 else 0.06
        else:
            gamma_sys = 0.06
    else:
        gamma_sys = 0.06
        print(f"  Warning: {s10_path} not found; using fallback systematic uncertainty.")

    # Combined uncertainty (quadrature)
    gamma_total = np.sqrt(gamma_stat**2 + gamma_sys**2 + gamma_rho_unc**2)

    # Asymmetric uncertainty: use combined total for both bounds
    gamma_lower = gamma_total
    gamma_upper = gamma_total
    ...
    density_scaling_slope = {
        ...
        'newtonian_prediction': {
            'value': float(newt_pred),
            'uncertainty': float(newt_err),
    ...
    s35_path = Path('results/outputs/step_13_covariance_validation.json')
    if not s35_path.exists():
        print(f"ERROR: Required input file not found: {s35_path}")
        print(f"Theoretical uncertainty requires covariance validation results from step_13.")
        raise RuntimeError("Missing required input: step_13_covariance_validation.json")
    
    with open(s35_path, 'r') as f:
        s35_data = json.load(f)
    offset_central = s35_data['covariance_aware_ttest']['difference_dex']
    offset_stat = s35_data['covariance_aware_ttest']['se_diff']  # from covariance-aware t-test

    # Systematic uncertainty: matching procedure variation
    # Estimated from difference between period-matched and 2D-matched residuals
    if s10_path.exists():
        with open(s10_path) as f:
            s10 = json.load(f)
        period_diff = s10['controls']['period_matched']['diff_mean']
        two_dim_diff = s10['controls']['period_and_bproxy_matched']['diff_mean']
        offset_sys = abs(period_diff - two_dim_diff)
    else:
        offset_sys = 0.06
        print(f"  Warning: {s10_path} not found; using fallback offset systematic uncertainty.")

    # Propagated (ρ_intra): use bootstrap CI half-width from step_13
    boot = s35_data.get('hierarchical_bootstrap', {})
    if boot:
        offset_rho_unc = (boot.get('ci_95_upper', 0.63) - boot.get('ci_95_lower', 0.50)) / 4
    else:
        offset_rho_unc = 0.06
    ...
    s36_path = Path('results/outputs/step_21_integrated_binary_control.json')
    if not s36_path.exists():
        print(f"ERROR: Required input file not found: {s36_path}")
        print(f"Theoretical uncertainty requires binary control results from step_21.")
...
def main():
    ...
    output_path = output_dir / 'step_33_theoretical_uncertainty.json'
