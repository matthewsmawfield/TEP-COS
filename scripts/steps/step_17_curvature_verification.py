#!/usr/bin/env python3
"""
Step 17: Density-Based Screening Competition Verification
...
def compute_screening_radius(M_obj_msun: float, rho_c_g_cm3: float = 20.0) -> float:
    ...
    Within this radius, the Temporal Topology flattens and Temporal Shear
    is suppressed. R_sol = (3M / 4πρ_c)^(1/3)
    """
...
def compute_gravitational_influence(M_msun: float, distance_m: float) -> float:
    ...
    In the continuous screening picture, the Temporal Shear contribution
    from a density well scales as M/d, representing its gravitational
    influence on the topology transition.
    """
    M_kg = M_msun * M_sun
...
def compute_shielding_fraction(
    ...
    """
    Compute shielding fraction from density-based Temporal Shear competition.

    The companion creates a region of suppressed Temporal Shear
    (φ ≈ φ_min(ρ_c)) that competes with the cluster's active Shear
    (φ ≈ φ_cluster) at the pulsar topology transition.
...
def main():
    print("=" * 80)
    print("DENSITY-BASED TEMPORAL SHEAR COMPETITION VERIFICATION")
    print("=" * 80)

    # TEP parameters
    rho_c = 20.0  # g/cm³

    # Pulsar saturation radius
    Mp = 1.4  # M_sun
    ...
    print(f"Saturation radius R_sol: {R_sol / 1000:.1f} km")
    ...
    print(f"\n3. TEMPORAL SHEAR COMPETITION AT PULSAR TOPOLOGY TRANSITION")
    print(f"-" * 40)
    print(
        f"Distance from companion to topology transition: {result['distance_to_boundary_m'] / 1000:.1f} km"
    )
    ...
    print(
        f"Shielding fraction: {result['shielding_fraction']:.3f} ({result['shielding_fraction'] * 100:.1f}%)"
    )
    ...
    if discrepancy < 0.15:  # Within 15 percentage points
        print(
            f"\nVERIFIED: Density-based Temporal Shear competition agrees with observation"
        )
        print(f"  The continuous screening mechanism is validated.")
    else:
    ...
    # Compile output
    results = {
        "screening_model": "Density-based Temporal Shear competition",
        "formula": "f_shield = (M_c/d) / (M_clust/R_core + M_c/d)",
        "tep_parameters": {
            "rho_c_g_cm3": rho_c,
            "pulsar_mass_msun": Mp,
            "pulsar_saturation_radius_km": R_sol / 1000,
        },
        "typical_binary": {
            "period_days": Pb,
    ...
    out_path = RESULTS_DIR / "step_17_screening_competition.json"
    ...
    print(f"")
    print(f"\nThis validates the TEP continuous screening mechanism for")
