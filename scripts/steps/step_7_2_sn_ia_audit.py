...
try:
    ...
    if dups > 0:
        ...
        print(f"WARNING: {dups} duplicate CIDs")
    else:
...
# ============================================================================
# 4. DISTRIBUTION CHECKS (Non-normality is expected for astrophysical populations)
# ============================================================================
...
# NOTE: Non-normal distributions are common in astrophysical data due to:
# - Selection effects (flux-limited surveys)
# - Intrinsic population heterogeneity
# - Measurement uncertainties
# We report normality tests but rely on non-parametric methods for robustness
if len(valid) < 5000:
    stat, p = shapiro(log_sigma)
    print(f"Log(σ) normality (Shapiro-Wilk): p = {p:.4f}")
    if p < SIGNIFICANCE_THRESHOLD:
        # Only flag as warning if both distributions are non-normal
        stat_mB, p_mB = shapiro(mB)
        if p_mB < SIGNIFICANCE_THRESHOLD:
            warnings_list.append("Both Log(σ) and mB distributions are non-normal - using non-parametric methods")
        else:
            print(f"  Note: Log(σ) non-normal but mB is normal - using Spearman correlation for robustness")
...
# ============================================================================
# 5. MATHEMATICAL CORRECTNESS (with robust methods)
# ============================================================================
...
# Spearman rank correlation (non-parametric, robust to non-normality)
rho, p_spear = spearmanr(valid['sigma_host'], mB)
print(f"\nSpearman correlation (non-parametric, robust to non-normality):")
print(f"  ρ = {rho:.6f}")
print(f"  p = {p_spear:.6e}")

# Outlier-robust correlation (Winsorized at 5%)
from scipy.stats import mstats
log_sigma_winsorized = mstats.winsorize(log_sigma, limits=[0.05, 0.05])
mB_winsorized = mstats.winsorize(mB, limits=[0.05, 0.05])
r_wins, p_wins = pearsonr(log_sigma_winsorized, mB_winsorized)
print(f"\nWinsorized correlation (5% outlier-robust):")
print(f"  r = {r_wins:.6f}")
print(f"  p = {p_wins:.6e}")

# Consistency check across methods
print(f"\nMethod consistency check:")
print(f"  Pearson vs Spearman: {abs(r - rho):.4f} (should be < 0.1)")
print(f"  Pearson vs Winsorized: {abs(r - r_wins):.4f} (should be < 0.1)")
if abs(r - rho) > 0.1:
    warnings_list.append("Large discrepancy between Pearson and Spearman (non-linearity suspected)")
if abs(r - r_wins) > 0.1:
    warnings_list.append("Large discrepancy with winsorized correlation (outliers influential)")
...
# ============================================================================
# 7. OUTLIER DETECTION (Expected in astrophysical samples)
# ============================================================================
print("\n7. OUTLIER DETECTION")
print("-"*70)
...
# Expected outliers in normal distribution (0.3% per tail, ~0.6% total)
n_expected_outliers = int(len(valid) * 0.003 * 2)
print(f"  Expected (normal approx): ~{n_expected_outliers}")

if sigma_outliers > 0:
    outlier_idx = z_sigma > 3
    print(f"  High-σ outliers: {valid[outlier_idx]['CID'].tolist()}")
    # Only warn if significantly more outliers than expected
    if sigma_outliers > max(3, n_expected_outliers * 2):
        warnings_list.append(f"{sigma_outliers} σ outliers detected (more than 2x expected)")
    else:
        print(f"  Note: Outlier count consistent with Gaussian expectation")

if mb_outliers > 0:
    # Only warn if significantly more outliers than expected  
    if mb_outliers > max(3, n_expected_outliers * 2):
        warnings_list.append(f"{mb_outliers} mB outliers detected (more than 2x expected)")
    else:
        print(f"  Note: mB outlier count consistent with Gaussian expectation")
...
verdict_match = step7_verdict in ['tep_consistent', 'tep_consistent_with_mass_ambiguity'] if correct_verdict == 'tep_consistent' else json_res['verdict'] == correct_verdict
...
if len(warnings_list) > 0:
    print(f"\nWARNING: {len(warnings_list)} WARNINGS:")
