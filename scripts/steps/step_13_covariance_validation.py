#!/usr/bin/env python3
"""
Step 13: Covariance-Aware Statistical Validation
...
def estimate_rho_intra_from_data(gc_df: pd.DataFrame) -> float:
    """
    Estimate intra-cluster correlation coefficient from actual data.

    Uses the ratio of between-cluster variance to total variance
    (ANOVA-style ICC estimate). Falls back to 0.3 if estimation fails.
    """
    clusters = gc_df.groupby('cluster')
    cluster_means = []
    cluster_vars = []
    for cluster_name, cluster_df in clusters:
        vals = cluster_df['logPdot_abs'].dropna().values
        if len(vals) >= 2:
            cluster_means.append(np.mean(vals))
            cluster_vars.append(np.var(vals, ddof=1))

    if len(cluster_means) < 2:
        print("  Warning: insufficient clusters to estimate rho_intra; using fallback 0.3")
        return 0.3

    # ANOVA-style ICC(1) estimate
    n_bar = np.mean([len(c) for _, c in clusters if len(c) >= 2])
    ms_between = np.var(cluster_means, ddof=1) * n_bar
    ms_within = np.mean(cluster_vars) if cluster_vars else 0
    ...
    icc = (ms_between - ms_within) / (ms_between + (n_bar - 1) * ms_within)
    rho_intra = max(0.0, min(0.9, icc))  # Clamp to [0, 0.9]

    print(f"  Data-driven rho_intra estimate: {rho_intra:.3f}")
    return rho_intra
...
def hierarchical_bootstrap(
    ...
    Proper hierarchical bootstrap:
    1. Resample clusters with replacement (captures between-cluster variation)
    2. Within each resampled cluster, resample individuals with replacement
       (captures within-cluster variation)
    3. No artificial noise injection needed — resampling at both levels
       correctly reproduces the hierarchical correlation structure.
    """
    clusters = gc_df.groupby('cluster')
    ...
    for b in range(n_bootstrap):
        ...
        for c in resampled_clusters:
            cluster_data = clusters.get_group(c)['logPdot_abs'].dropna().values
            # Within each cluster, resample individuals with replacement
            resampled = np.random.choice(cluster_data, size=len(cluster_data), replace=True)
...
def permutation_test_null(
    ...
    """
    Cluster-aware permutation test for the null hypothesis of no GC-field difference.

    Under H0, the environment label (GC vs field) is independent of the measured
    logPdot_abs value. To preserve within-cluster correlation structure, we treat
    each cluster as a unit: we pool all GC cluster means and field values, then
    randomly assign which cluster means are "GC" and which are pooled with field.

    This is more conservative than shuffling individual values (which destroys
    within-cluster correlation) and appropriately tests the null given the data
    structure.
    """
    # Cluster means for GC (treat clusters as units)
    gc_cluster_means = gc_df.groupby('cluster')['logPdot_abs'].mean().values
    field_values = field_df['logPdot_abs'].dropna().values

    # Observed difference: cluster-mean GC vs field individual mean
    observed_diff = np.mean(gc_cluster_means) - np.mean(field_values)

    # Pool all units: GC cluster means + field individual values
    pooled = np.concatenate([gc_cluster_means, field_values])
    n_gc_units = len(gc_cluster_means)

    perm_diffs = []

    for _ in range(n_permutations):
        # Shuffle the pooled units
        shuffled = np.random.permutation(pooled)
        # First n_gc_units are "GC", rest are "field"
        perm_gc = shuffled[:n_gc_units]
        perm_field = shuffled[n_gc_units:]
    ...
    return {
        ...
        'note': (
            'Permutation test uses GC cluster means (not individual pulsars) to preserve '
            'within-cluster correlation structure. observed_difference here differs from '
            'covariance_aware_ttest.difference_dex because the latter uses all GC individuals.'
        ),
    }
...
def main():
    ...
    # Estimate rho_intra from actual data instead of hardcoding
    rho_intra = estimate_rho_intra_from_data(gc_df)
    ...
    boot = hierarchical_bootstrap(gc_df, field_df, n_bootstrap=BOOTSTRAP_SAMPLES_MEDIUM)
    ...
    # Save results
    # NOTE: The covariance-aware t-test is the PRIMARY hypothesis test.
    # All other tests are ROBUSTNESS CHECKS that validate stability and
    # assumptions. They are NOT independent confirmations and should NOT
    # be interpreted as multiplying the statistical evidence.
    output = {
        'primary_test': {
            'name': 'covariance_aware_ttest',
            'description': 'Primary hypothesis test: GC vs Field mean difference accounting for intra-cluster correlation',
            'p_value': cov_test['p_value'],
            'significance_sigma': abs(cov_test['t_statistic']),
            'effect_size_dex': cov_test['difference_dex']
        },
        'robustness_checks': {
            'leave_one_cluster_out': {
                'description': 'Stability check: result not driven by single clusters',
                'assessment': loocv['stability_assessment'],
                'relative_instability': loocv['relative_instability'],
                'note': 'Does NOT provide independent p-value; validates stability of primary result'
            },
            'hierarchical_bootstrap': {
                'description': 'Uncertainty quantification: confidence intervals for effect size',
                'ci_95_lower': boot['ci_95_lower'],
                'ci_95_upper': boot['ci_95_upper'],
                'note': 'Does NOT provide independent p-value; quantifies uncertainty in primary result'
            },
            'permutation_test': {
                'description': 'Null validation: observed difference unlikely under null hypothesis',
                'p_value': perm['p_value'],
                'note': 'Correlated with primary test; validates null model, not independent confirmation'
            },
            'density_scaling_loocv': {
                'description': 'Stability check: density scaling slope robust to cluster removal',
                'assessment': density_loocv['stability_assessment'],
                'relative_instability': density_loocv['relative_instability'],
                'note': 'Separate analysis of slope stability; not a test of the same hypothesis'
            }
        },
        ...
        'metadata': {
            ...
            'multiple_testing_note': 'Only the covariance-aware t-test is a formal hypothesis test. Other analyses are robustness checks and their p-values/assessments should NOT be combined for family-wise error correction.'
        }
    }
    
    os.makedirs('results/outputs', exist_ok=True)
    with open('results/outputs/step_13_covariance_validation.json', 'w') as f:
        json.dump(output, f, indent=2)
    
    print("\n" + "=" * 70)
    print("Results saved to: results/outputs/step_13_covariance_validation.json")
    print("=" * 70)
