...
def analyze_density_scaling(clusters: Dict[str, CMCParser], cmc_pulsars_by_cluster: Dict[str, pd.DataFrame]) -> Dict:
    """
...
def analyze_binary_behavior(cmc_pulsars: pd.DataFrame) -> Dict:
    ...
    2. Layer 2: The binary companion creates a local time domain around the pulsar
    3. Layer 3: The pulsar's own screening boundary anchors to its local domain
    
    When we observe from Earth, we look through nested time domains:
...
def render_falsification_verdict(
    ...
    if excess_matches and density_matches:
        ...
        slope_sigma = abs(density_test["cmc_slope"] - density_test["observed_slope"]) / \
                     np.sqrt(density_test["cmc_slope_error"]**2 + density_test["observed_error"]**2)
...
def main_analysis():
    ...
    if combined_cmc is not None:
        ...
        print(f"  Agreement:                          Within {abs(cmc_excess - 2.1):.2f} dex")
    else:
    ...
    # TEP explains this through nested time domains.
    # ========================================================================
    print("\n" + "=" * 70)
