...
Usage:
    python download_cmc_data.py [--cluster 47_Tuc] [--all]
...
def download_with_progress(url: str, params: Dict, timeout: int = 600) -> Optional[bytes]:
    ...
    try:
        # Stream the response
        response = requests.post(url, data=params, timeout=timeout, stream=True)
...
def main():
    ...
    parser = argparse.ArgumentParser(
        ...
        epilog="""
Examples:
  python download_cmc_data.py --all
  python download_cmc_data.py --cluster 47_Tuc
  python download_cmc_data.py --cluster Terzan_5 --force
        """
    )
    ...
    # Exit code
    sys.exit(0 if total_success == len(clusters) else 1)
