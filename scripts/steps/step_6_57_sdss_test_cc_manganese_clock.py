...
def analyze_manganese(df):
    ...
    binned = df_sub.groupby('r_bin')['mn_fe'].agg(['mean', 'sem', 'count'])
    ...
    out_path = os.path.join(FIGURES_DIR, 'sdss_test_cc_manganese.png')
...
def main():
    ...
    out_path = os.path.join(RESULTS_DIR, 'sdss_test_cc_results.json')
    if results:
        def default(o):
