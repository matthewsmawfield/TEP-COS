#!/usr/bin/env python3
"""
Step 27: Monte Carlo Validation of Statistical Methods
...
def generate_synthetic_data(n_gc_clusters=29, n_per_cluster=6, n_field=198, 
    ...
    # Load field population parameters from step_02 if available
    s10_path = Path("results/outputs/step_02_pulsar_population_controls.json")
    if s10_path.exists():
        with open(s10_path) as f:
            s10 = json.load(f)
        field_mean = s10["base_log10_abs_pdot"]["field_mean"]
        # Estimate field std from data if available, else fallback
        import pandas as pd
        csv_path = Path("results/outputs/step_02_pulsar_population_controls.csv")
        if csv_path.exists():
            df = pd.read_csv(csv_path)
            field_std = df[df["environment"] == "field"]["logPdot_abs"].std()
        else:
            field_std = 0.64
    else:
        field_mean = -19.76
        field_std = 0.64
        print("  Warning: step_02 not found; using fallback field population parameters.")

    # Field population (reference)
    field_logpdot = np.random.normal(field_mean, field_std, n_field)
    ...
    cluster_mean_base = field_mean + effect_size  # Shifted by effect size
    ...
    for i, cluster in enumerate(cluster_names):
        ...
        cluster_values = np.random.normal(cluster_means[i], field_std * np.sqrt(1 - rho_intra), n_this_cluster)
...
def main():
    ...
    with open('results/outputs/step_27_monte_carlo_validation.json', 'w') as f:
        json.dump(output, f, indent=2, default=lambda x: bool(x) if isinstance(x, np.bool_) else x)
    
    print(f"\nResults saved to: results/outputs/step_27_monte_carlo_validation.json")
    print("=" * 70)
