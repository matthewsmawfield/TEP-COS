#!/usr/bin/env python3
"""
Step 19: Validate Derived Shielding Formula
...
through the continuous Temporal Topology framework.
...
def compute_derived_shielding(
    ...
    with φ ≈ φ_min(ρ_c) inside its saturation radius.
    The cluster provides enhanced background with active Temporal Shear
    (φ ≈ φ_cluster).
    The pulsar topology transition sees weighted average of both.
    ...
    return {
        ...
        "distance_to_topology_m": float(d),
...
def validate_against_observations():
    ...
    result = compute_derived_shielding(
        M_comp_msun=0.2, orbital_separation_m=a, R_sol_m=R_sol
    )
    ...
    output = {
        ...
        "derivation": {
            ...
            "physics": [
                "Companion creates region of suppressed Temporal Shear with φ ≈ φ_min(ρ_c)",
                "Cluster provides enhanced background with active Temporal Shear",
                "Pulsar topology transition sees weighted average of both",
                "Weights are mass/distance (gravitational influence)",
            ],
        },
        ...
        "mass_trend": {
            "direction": "positive",
            "meaning": "Higher companion mass gives stronger Temporal Shear suppression",
            "correct": True,
        },
    }

    output_path = Path("results/outputs/step_19_derived_shielding.json")
    ...
    print()
    print(
        "This derivation follows from TEP continuous field equations, not empirical calibration."
    )
    print()
