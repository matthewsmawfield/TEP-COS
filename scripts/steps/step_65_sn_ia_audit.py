...
Dependencies:
- Requires outputs from step_62 and step_64
- Pantheon+ catalog
...
try:
    with open('results/outputs/step_62_sn_ia_mB_sigma.json', 'r') as f:
        step_62 = json.load(f)
    print(f"✓ Step 7.0 results loaded: r = {step_62['pearson']['r']:.4f}")
except Exception as e:
    print(f"✗ Step 7.0 results not found: {e}")
    step_62 = None

try:
    with open('results/outputs/step_63_robustness_analysis.json', 'r') as f:
        step_64 = json.load(f)
    print(f"✓ Step 7.1 results loaded: overall_robust = {step_64.get('overall_robust', False)}")
except Exception as e:
...
if step_62 is not None and step_64 is not None:
    r_orig = step_62['pearson']['r']
    # Compare with bootstrap mean from step_64 (nested under tests)
    bootstrap_data = step_64.get('tests', {}).get('bootstrap')
    if bootstrap_data and 'r_mean' in bootstrap_data:
        r_boot_mean = bootstrap_data['r_mean']
        ...
        print(f"   ⚠ Bootstrap r_mean not available in step_64 results")
...
# Save results
with open('results/outputs/step_65_sn_ia_audit.json', 'w') as f:
    json.dump(audit_results, f, indent=2)

print("\nResults saved to: results/outputs/step_65_sn_ia_audit.json")
