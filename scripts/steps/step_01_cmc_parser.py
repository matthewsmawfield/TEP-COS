...
class CMCParser:
    ...
    def parse_morepulsars(self) -> Optional[pd.DataFrame]:
        ...
        try:
            ...
            # DEDUPLICATE: initial.morepulsars.dat contains multiple time snapshots
            # per neutron star (tracked by id0). We keep the LAST snapshot per id0
            # to represent the most evolved state. Without deduplication, row counts
            # inflate by ~1000x (e.g., 1.4M rows for ~1,200 unique NS).
            if 'id0' in df.columns:
                n_before = len(df)
                df = df.groupby('id0').tail(1).copy()
                n_after = len(df)
                if n_before != n_after:
                    print(f"  Deduplicated {n_before} rows -> {n_after} unique neutron stars")
    ...
    def _compute_derived(self, df: pd.DataFrame) -> pd.DataFrame:
        ...
        if 'r' in df.columns:
            ...
            # Gravitational acceleration from enclosed mass
            # This is the physically correct Newtonian acceleration:
            # a(r) = G * M_enc(r) / r^2
            r_soft = np.sqrt(r**2 + 0.05**2)
            a_grav_3d = G * M_enc / r_soft**2

            # LINE-OF-SIGHT PROJECTION
            # Real MSPs have random orientations relative to the observer.
            # For isotropic directions, cos(theta) is uniform on [-1, 1].
            # We assign a fixed-seed random projection per pulsar so that
            # the variance of the line-of-sight field is captured correctly.
            # (Jensen's inequality: the mean of log|Pdot| shifts by the variance.)
            rng = np.random.RandomState(42)
            cos_theta = rng.uniform(-1, 1, size=len(df))
            a_los = a_grav_3d * cos_theta

            # Convert to m/s^2
            # a_grav_3d is in (km/s)^2 / pc;
            # 1 (km/s)^2 / pc = 1e3 / 3.086e13 m/s^2
            MS2_CONVERSION = 1e3 / 3.086e13
            df['a_grav_ms2'] = a_los * MS2_CONVERSION

            # Store physical radius and core radius for downstream mass-segregation cuts
            df['r_pc'] = r
            df['r_core_pc'] = r_c
