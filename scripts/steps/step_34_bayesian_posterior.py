#!/usr/bin/env python3
"""Step 34: Bayesian Posterior Analysis
=======================================

Bayesian hierarchical modeling for density scaling.
...
def bayesian_analysis_pymc_simulation():
    ...
    # - Newtonian predicts ~0.75 (literature consensus 0.748 ± 0.039)
    ...
    if not s33_path.exists():
        ...
        print(f"Bayesian analysis requires hierarchical density results from step_12.")
    ...
    observed_mean = s33_data['model_b_mixed_slope']
    observed_se = s33_data['model_b_mixed_error']

    # Load Newtonian prediction from literature consensus
    s48_path = Path('results/outputs/step_14_cmc_literature.json')
    if s48_path.exists():
        with open(s48_path, 'r') as f:
            s48_data = json.load(f)
        newtonian_pred = s48_data.get('comparison', {}).get('cmc_predicted_slope', 0.748)
    else:
        newtonian_pred = s33_data.get('newtonian_predicted_slope', 0.748)
    ...
    density_scaling_posterior = {
        ...
        'prior': {
            ...
            'std': float(prior_std),
            'rationale': 'Weakly informative: encompasses Newtonian (~0.75) and TEP (~0.35) predictions'
        },
        'likelihood': {
        ...
        'hypothesis_testing': {
            'newtonian_prediction': float(newtonian_pred),
            'p_newtonian': float(1 - stats.norm.cdf(newtonian_pred, loc=posterior_mean, scale=posterior_std)),
    ...
    print(f"\nHypothesis Testing:")
    print(f"  P(Γ > {newtonian_pred:.2f} | data) = {density_scaling_posterior['hypothesis_testing']['p_newtonian']:.4f}")
    ...
    if not s35_path.exists():
        ...
        print(f"Bayesian analysis requires covariance validation results from step_13.")
    ...
    if observed_offset is None or observed_offset_se is None:
        raise RuntimeError("Required fields 'gc_field_mean_offset_dex' or 'gc_field_offset_se_dex' not found in step_13_covariance_validation.json")
    ...
    gc_field_posterior = {
        ...
        'likelihood': {
            ...
            'source': 'Covariance-aware t-test (step_13)'
        },
        'posterior': {
    ...
    # Intercept: use field MSP mean from step_02 if available, otherwise approximate
    s10_path = Path('results/outputs/step_02_pulsar_population_controls.json')
    if s10_path.exists():
        with open(s10_path) as f:
            s10 = json.load(f)
        intercept = s10['base_log10_abs_pdot']['field_mean']
    else:
        intercept = -19.5  # Fallback approximate field MSP mean
        print("  Warning: step_02 not found, using approximate intercept -19.5")
    ...
    results = {
        ...
        'model_comparison': {
            'notes': 'Savage-Dickey density ratio for nested model comparison: BF_01 = posterior_density(theta_0) / prior_density(theta_0)',
            'newtonian_log_bayes_factor': float(
                np.log(stats.norm.pdf(newtonian_pred, loc=posterior_mean, scale=posterior_std)) -
                np.log(stats.norm.pdf(newtonian_pred, loc=prior_mean, scale=prior_std))
            ),
            'tep_posterior_probability': float(
                stats.norm.cdf(0.45, loc=posterior_mean, scale=posterior_std) -
                stats.norm.cdf(0.30, loc=posterior_mean, scale=posterior_std)
            ),
            'tep_prior_probability': float(
                stats.norm.cdf(0.45, loc=prior_mean, scale=prior_std) -
                stats.norm.cdf(0.30, loc=prior_mean, scale=prior_std)
            ),
    ...
    output_path = output_dir / 'step_34_bayesian_posterior.json'
...
def generate_latex_summary(results):
    ...
    newt_pred = gamma['hypothesis_testing']['newtonian_prediction']
    print(f"\nModel Comparison:")
    print(f"P($\\Gamma > {newt_pred:.2f}$ | data) = {gamma['hypothesis_testing']['p_newtonian']:.4f}")
