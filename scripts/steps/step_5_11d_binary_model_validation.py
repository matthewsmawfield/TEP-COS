#!/usr/bin/env python3
"""
Step 5.11d: Binary Model Validation - Test Period/Mass Predictions
...
Model Predictions:
1. Longer orbital period → less shielding → higher log Ṗ (positive correlation)
2. More massive companion → more shielding → lower log Ṗ (negative correlation)
...
def compute_shielding_fraction(Pb_days: float, Mc_msun: float, Mp_msun: float = 1.4) -> float:
    """
    Compute predicted shielding fraction from density-based screening competition.
    
    Uses the chameleon screening boundary formula from step_5_11b:
    f_shield = (M_c/d) / (M_clust/R_core + M_c/d)
    
    where:
    - M_c = companion mass
    - d = distance from companion to pulsar screening boundary
    - M_clust = cluster mass
    - R_core = cluster core radius
    
    This derives from TEP chameleon screening where competing screened regions
    contribute to the boundary field in proportion to their gravitational influence.
    """
    # Physical constants
    G = 6.674e-11  # m³ kg⁻¹ s⁻²
    M_sun = 1.989e30  # kg
    ...
    # Pulsar screening radius (density-based)
    Mp_kg = Mp_msun * M_sun
    ...
    # Shielding fraction from chameleon boundary competition
            ...
            try:
                ...
                if P_s < 0.030 and Pb_days is not None and Pb_days > 0:
                    binaries.append({
                        ...
                        'log_Pdot': log_Pdot,
                        'Pb_days': Pb_days,
                        'm2_msun': m2_msun if m2_msun else 0.2  # Default companion mass
                    })
...
def main():
    ...
    shielding = np.array([compute_shielding_fraction(b['Pb_days'], b['m2_msun']) for b in binaries])
    ...
    for pmin, pmax in period_bins:
        ...
        if mask.sum() > 0:
            ...
            print(f"{pmin:.1f}-{pmax:.1f} d     | {mask.sum():<5} | {mean_pdot:<12.3f} | {mean_shield:<12.2f}")
    ...
    for mmin, mmax in mass_bins:
        ...
        if mask.sum() > 0:
            mean_pdot = log_Pdot[mask].mean()
            mean_shield = shielding[mask].mean()
            print(f"{mmin:.2f}-{mmax:.2f} M☉   | {mask.sum():<5} | {mean_pdot:<12.3f} | {mean_shield:<12.2f}")
    ...
    results = {
        ...
        "correlations": {
            ...
            "partial_shielding_vs_logPdot": {"r": float(r_partial), "p": float(p_partial)}
        },
        "predictions": {
    ...
    out_path = RESULTS_DIR / "step_5_11d_model_validation.json"
