...
/**
 * HTML to Markdown Converter for TEP-COS Site
...
class HTMLToMarkdownConverter {
        ...
        rowMatches.forEach((row, index) => {
            ...
            if (cells) {
                const cellTexts = cells.map(cell => {
                    // Convert subscripts and superscripts to unicode/text FIRST
                    let text = cell;
                    text = text.replace(/<sup>([^<]*)<\/sup>/gi, (m, p1) => {
                        // Convert common superscripts
                        const superMap = {'0':'⁰','1':'¹','2':'²','3':'³','4':'⁴','5':'⁵','6':'⁶','7':'⁷','8':'⁸','9':'⁹','-':'⁻','+':'⁺','−':'⁻'};
                        return p1.split('').map(c => superMap[c] || c).join('');
                    });
                    text = text.replace(/<sub>([^<]*)<\/sub>/gi, (m, p1) => {
    ...
    extractMetadata(html) {
        ...
        if (doiDivMatch) {
            ...
            let rawDoi = doiDivMatch[1].replace(/<!--[\s\S]*?-->/g, '').replace(/<[^>]+>/g, '').trim();
            // Remove "DOI:" prefix if present
            doi = rawDoi.replace(/^DOI:\s*/i, '');
        }
